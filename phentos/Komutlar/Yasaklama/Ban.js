...
module.exports = {
    Isim: "ban",
    Komut: ["yasakla","yargı"],
    Kullanim: "yasakla @PHENTOS/ID <sebep>",
  ...
  if(!phentos.Roller.banHammer) return message.channel.send("Sistemsel hata: Rol bulunamadı veya rol bilgileri girilemedi.").then(x => x.delete({timeout: 5000}));
  if(!phentos.Roller.banHammer.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`).then(x => x.delete({timeout: 5000}));
  ...
  if (args[0] && (args[0].includes('bilgi') || args[0].includes('info'))) {
    if(!args[1] || isNaN(args[1])) return message.channel.send(`Hata: Lütfen geçerli yasaklanmış üye Id girin!  __Örn:__  \`${client.sistem.a_Prefix}ban bilgi @phentos/ID\``).then(x => x.delete({timeout: 5000}));;
  ...
  if (!reason) return message.channel.send(`Hata: Lütfen bir sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @PHENTOS/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  let ceza = {
    No: cezano,
    Cezalanan: victim.id,
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      message.channel.send(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @PHENTOS/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
    };
    return message.reply(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @PHENTOS/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  };
