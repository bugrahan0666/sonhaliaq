...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!message.member.roles.cache.has(Pen1) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutu kullanabilmek için yeterli yetkiye sahip değilsin.`).then(sil => sil.delete({timeout: 5000}));
    ...
    if(uye.manageable) if(IkinciTag) message.guild.members.cache.get(uye.id).setNickname(`${IkinciTag} İsim | Yaş`).catch();
    else if(Pen1.Tag) message.guild.members.cache.get(uye.id).setNickname(`${Tag1} İsim | Yaş`).catch();
