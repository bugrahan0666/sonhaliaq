...
module.exports = {
    Isim: "uyar",
    Komut: ["uyarı","uyar"],
    Kullanim: "uyarı @phentos/ID <uyarı açıklaması>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!phentos.Roller.warnHammer) return message.channel.send("Sistemsel hata: Rol bulunamadı veya rol bilgileri girilemedi.").then(x => x.delete({timeout: 5000}));
    if(!phentos.Roller.warnHammer.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`).then(x => x.delete({timeout: 5000}));
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    let reason = args.splice(1).join(" ");
    if(!uye || !reason) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id girin!  __Örn:__  \`${client.sistem.a_Prefix}uyarı @phentos/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
    ...
    message.react("✅")
    if(phentos.Kanallar.uyarıLogKanali && client.channels.cache.has(phentos.Kanallar.uyarıLogKanali)) client.channels.cache.get(phentos.Kanallar.uyarıLogKanali).send(embed.setDescription(`$${uye} (\`${uye.id}\`) üyesi, ${message.author} (\`${message.author.id}\`) tarafından **${reason}** nedeniyle uyarıldı.`).setFooter(client.altbaslik + ` • Ceza Numarası: #${cezano}`)).catch();
